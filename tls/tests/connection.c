                               ...
                               gconstpointer   data)
{
  ...
  istream = g_io_stream_get_input_stream (test->client_connection);
  nread = g_input_stream_read (istream, &buf, 1, NULL, &error);
  g_assert_no_error (error);
  g_assert_cmpint (nread, <=, 0);
  
  ostream = G_POLLABLE_OUTPUT_STREAM (g_io_stream_get_output_stream (test->client_connection));

  /* First write will succeed. */
  written = g_pollable_output_stream_write_nonblocking (ostream,
                                                        &byte,
                                                        1,
                                                        NULL,
                                                        &error);
  ...
  /* Subsequent writes must fail with CONNECTION_CLOSED. */
  written = g_pollable_output_stream_write_nonblocking (ostream,
                                                        &byte,
                                                        1,
                                                        NULL,
