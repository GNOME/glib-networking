...
{
        g_debug ("C_FindObjects %lu %ld %s %lu", mock_search_iterator, mock_search_template_class, mock_search_template_label, ulMaxObjectCount);
        if (CK_FALSE == pkcs11_mock_initialized)
                return CKR_CRYPTOKI_NOT_INITIALIZED;
...
CK_DEFINE_FUNCTION(CK_RV, C_SignInit)(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
        ...
        if (CKM_RSA_PKCS_PSS == pMechanism->mechanism)
        {
                ...
                g_debug ("Hash algo: 0x%04lx, Mask: 0x%02lx, Salt length: %lu", params->hashAlg, params->mgf, params->sLen);
...
{
        g_debug ("C_Sign key: %lu, data: %p in-len: %lu, out-size: %lu", pkcs11_mock_sign_key, pData, ulDataLen, *pulSignatureLen);
