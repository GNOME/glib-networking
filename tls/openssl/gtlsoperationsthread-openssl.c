                                           ...
                                           GError                   **error)
{
  ...
  if (is_client (self) && !self->op_own_certificate)
    {
      g_tls_operations_thread_base_request_certificate (base,
                                                        cancellable,
  ...
  *peer_certificate = get_peer_certificate (self);
   ...
   */
  if (ret > 0 &&
      (!*peer_certificate || !g_tls_operations_thread_base_verify_certificate (G_TLS_OPERATIONS_THREAD_BASE (self),
                                                                               *peer_certificate,
                                                                               context)))
    {
