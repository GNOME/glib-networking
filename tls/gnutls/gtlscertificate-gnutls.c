                       ...
                       GError                **error)
{
  ...
  if (gnutls_err != GNUTLS_E_SUCCESS)
    {
      g_set_error (error, G_IO_ERROR, G_TLS_ERROR_MISC, "Failed to export certificate: %s", gnutls_strerror (gnutls_err));
      return NULL;
    }
  ...
  if (g_tls_certificate_gnutls_has_key (cert))
    {
      ...
      gnutls_err = gnutls_x509_privkey_export2_pkcs8 (cert->key, GNUTLS_X509_FMT_DER, NULL, GNUTLS_PKCS_PLAIN, &privkey_data);
      if (gnutls_err != GNUTLS_E_SUCCESS)
        {
          g_set_error (error, G_IO_ERROR, G_TLS_ERROR_MISC, "Failed to export private-key: %s", gnutls_strerror (gnutls_err));
