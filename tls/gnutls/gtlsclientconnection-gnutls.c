  ...
  if (*pcert_length == 0)
    {
      ...
      if (g_tls_connection_gnutls_request_certificate (conn, g_tls_connection_base_get_certificate_error (tls)))
        g_tls_connection_gnutls_get_certificate (conn, pcert, pcert_length, pkey);
                                                   ...
                                                   GError             **error)
{
  ...
  status = G_TLS_CONNECTION_BASE_CLASS (g_tls_client_connection_gnutls_parent_class)->complete_handshake (tls, error);
